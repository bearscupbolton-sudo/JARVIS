...
export class DatabaseStorage implements IStorage {
  ...
  private async _bakeOffItemInTx(
    tx: any,
    boxDoughs: any[],
    box: "box1" | "box2" | "box3",
    target: string,
    ...
    userId: string | null,
    now: Date,
  ): Promise<{ actuallyBaked: number; remaining: number; affectedDoughIds: number[]; logId: number | null }> {
      ...
      if (take >= doughTargetPieces) {
        if (hasShapings) {
          ...
          if (remainingTotal <= 0) {
            ...
            dough.shapings = newShapings;
            dough.proofPieces = remainingTotal;
            dough.totalPieces = remainingTotal;
          }
        } else {
          await tx.update(laminationDoughs).set({
            status: "baked", bakedAt: now, bakedBy: userId,
        ...
        if (hasShapings) {
          ...
          dough.proofPieces = newPieces;
          dough.totalPieces = newPieces;
        }
      }
    }
  ...
  async bakeOffFromBox(box: "box1" | "box2" | "box3", pastryName: string, count: number, userId: string | null): Promise<{ actuallyBaked: number; remaining: number; affectedDoughIds: number[]; logId: number | null }> {
    ...
    return await db.transaction(async (tx) => {
      const boxDoughs = await tx.select().from(laminationDoughs)
        ...
        .orderBy(laminationDoughs.createdAt)
        .for("update");
      return this._bakeOffItemInTx(tx, boxDoughs, box, target, count, userId, new Date());
    });
  }

  async bakeOffBatchFromBox(
    box: "box1" | "box2" | "box3",
    items: Array<{ pastryName: string; count: number }>,
    userId: string | null,
  ): Promise<Array<{ pastryName: string; actuallyBaked: number; remaining: number; affectedDoughIds: number[]; logId: number | null }>> {
    const normalize = (s: string | null | undefined) => {
      if (!s) return "";
      const t = String(s).trim().replace(/\s+/g, " ").toLowerCase();
      return t.split(" ").map(w => w.length === 0 ? w : w[0].toUpperCase() + w.slice(1)).join(" ");
    ...
    const cleaned = items
      .map(it => ({ pastryName: normalize(it.pastryName), count: Math.max(0, Math.floor(Number(it.count) || 0)) }))
      .filter(it => it.pastryName && it.count > 0);
    ...
    return await db.transaction(async (tx) => {
      const boxDoughs = await tx.select().from(laminationDoughs)
        ...
        .for("update");
      const now = new Date();
      const results: Array<{ pastryName: string; actuallyBaked: number; remaining: number; affectedDoughIds: number[]; logId: number | null }> = [];
      for (const it of cleaned) {
        const r = await this._bakeOffItemInTx(tx, boxDoughs, box, it.pastryName, it.count, userId, now);
        results.push({ pastryName: it.pastryName, ...r });
      }
      return results;
    });
  }
  ...
  async activateProductionTask(jobId: number, opts: { notes?: string; department?: string; date?: string } = {}): Promise<TaskListItem | null> {
    ...
    let [list] = await db.select().from(taskLists)
      .where(and(eq(taskLists.department, department), eq(taskLists.date, date), eq(taskLists.status, "active")))
      .limit(1);

    if (!list) {
      [list] = await db.insert(taskLists).values({
        title: `${department.charAt(0).toUpperCase() + department.slice(1)} Tasks - ${date}`,
        department,
        date,
        status: "active",
        autoGenerated: true,

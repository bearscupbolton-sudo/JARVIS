...
import { getUserFromReq, createOvenTimersForItem } from "./routes/_helpers";
import { registerLaminationRoutes } from "./routes/lamination";
import { registerWholesaleRoutes } from "./routes/wholesale";
  ...
  app.get("/api/pastry-items/:id/cost-gaps", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (!passport) {
        ...
        if (!passport.motherRecipeId) {
          ...
          if (recipe) {
            ...
            const ingredients: RecipeIngredient[] = Array.isArray(recipe.ingredients) ? (recipe.ingredients as RecipeIngredient[]) : [];
      ...
      res.json({
        ...
        currentCost: costResult?.totalCost ?? null,
        dataCompleteness: costResult?.dataCompleteness ?? "none",
      });
    } catch (err: any) {
      res.status(500).json({ message: err.message });
    }

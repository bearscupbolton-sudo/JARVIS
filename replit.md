...
*   **The Firm (Financial Hub):** Owner-only forensic-level financial reconciliation system with overview, accounts, ledger, obligations, payroll, cash management, and sales tax reporting (from Square), including Jarvis AI financial analysis. Sales Tax tab pulls real-time order data from Square Orders API, showing total tax collected, effective rate, tax categories, daily breakdown with expandable per-day tax detail, gross/net sales, discounts, refunds, and tips.

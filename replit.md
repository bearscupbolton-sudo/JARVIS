...
*   **Developer Mode & Dev Feedback:** Owner-only toggle in Team Management that enables a global feedback system. When active, any team member can triple-tap anywhere on screen to open a feedback dialog to report bugs, suggest improvements, or share ideas. Submissions are processed by Jarvis AI to auto-generate titles, categories (UI/Production/Scheduling/etc.), priorities (low/medium/high/critical), and summaries. Owner reviews all feedback on a dedicated `/dev-feedback` page with status management (open/reviewed/in_progress/resolved/dismissed), filters, and expandable detail views with AI analysis. A subtle pulsing green dot in the bottom-left corner indicates when dev mode is active. Tables: `app_settings`, `dev_feedback`. Endpoints: `GET/POST /api/app-settings/dev-mode`, `POST /api/dev-feedback`, `GET/PATCH/DELETE /api/dev-feedback/:id`.

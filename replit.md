...
*   **JMT (Jarvis Menu Theater):** Creative control board for all menu display management. Owner-only page (`/jmt`) with Command Center (stats, live display overview, quick links), Menu Library (upload/edit/delete menu designs from Canva, category/orientation tagging, grid/list views, full-size preview), Display Matrix (15 configurable display slots with menu assignment, orientation/rotation settings, 86'd overlay toggle, go-live/take-offline controls), and Jarvis Recommends (AI-powered menu strategy suggestions). Schema: `jmt_menus` (menu image assets), `jmt_displays` (15 slots with slot number, assigned menu, orientation, rotation, live status), `jmt_display_history` (audit trail). Public display endpoints: `/menu/:slot` renders assigned menu image with rotation support and 86'd item overlay; `/api/jmt/screen/:slot` serves display data without auth. All 15 slots (including 1 and 2) are fully dynamic and controlled via JMT Display Matrix.

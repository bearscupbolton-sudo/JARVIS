...
*   **Comprehensive Bakery Management:**
    ...
    *   **Lightning-Offset Reconciliation:** Template-based bank transaction matching in Reconcile tab. Lightning button (⚡) on each bank transaction fetches auto-suggestions: vendor template match → Debit COGS/Credit Cash, or accrual placeholder match → Debit Accrued Liabilities (2100)/Credit Cash (1010) to prevent double-counting. Route: `/api/firm/reconcile/suggest`. Confidence scoring (95% exact, 85% vendor, 80% close, 60% vendor-only). Priority tiers: CapEx candidate → Lodging/Project Tag → Accrual offset → Vendor template → Learned rule → None.
    *   **Project Tagging (Lodging Intercept):** When Lightning-Offset detects a hotel/motel/Airbnb charge via `isLodgingCharge()` + `LODGING_KEYWORDS`, it returns `type: "project_tag_required"` with default options (Daily Operations COA 6140, Emergency/Maintenance COA 6070, Personal/dismiss) plus any active projects from `project_metadata`. Frontend intercept in Reconcile tab shows amber alert with option buttons. Table: `project_metadata` (CRUD routes), `firm_transactions.projectId` FK. Route: `/api/firm/transactions/:id/tag-project`. COA 6140 (Travel & Lodging) seeded. Categories `travel_lodging` and `repairs` added to category enum.

...
*   **BC Wholesale Portal:** Forward-facing wholesale portal with PIN-based authentication (`server/wholesale-auth.ts`), separate session (`wholesaleCustomerId`). Features: catalog browsing, order builder with template quick-load, recurring order templates by day of week, order history with status tracking. Order submission triggers owner message + calendar event. Owner admin page (`/wholesale-admin`) for managing customers, catalog items, and order statuses. DB tables: `wholesale_customers`, `wholesale_catalog_items`, `wholesale_orders`, `wholesale_order_items`, `wholesale_recurring_templates`, `wholesale_recurring_template_items`. Frontend pages in `client/src/pages/wholesale/`.

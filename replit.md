...
Team gaming arcade at `/starkade`. Locked when user is clocked in (active time entry); shows Iron Man-themed "suit up" message. Access check via `GET /api/starkade/access` (returns `{ locked, message }`). 4 built-in games: Lightning Reflexes (reaction), Bakery Memory (memory match), Baker's Brainteaser (quiz), Pastry Scramble (word scramble). Each game type has its own scoring engine. Unified points system across all games. Database: `starkade_games` (id, name, type, source builtin/ai, config JSONB, playCount, createdBy, status), `starkade_game_sessions` (id, gameId, userId, score, points, metadata JSONB). API: `GET /api/starkade/games`, `POST /api/starkade/games/:id/play` (submit score), `GET /api/starkade/leaderboard/global`, `GET /api/starkade/leaderboard/game/:id`, `GET /api/starkade/recent`. **AI Game Generator**: `POST /api/starkade/games/generate` (prompt → AI generates quiz/word/memory/reaction game config), `POST /api/starkade/games/save` (save generated game). Games are manager-deletable via `DELETE /api/starkade/games/:id`. Sidebar nav entry with Gamepad2 icon.

...
import {
  ...
  CheckCircle2, AlertCircle, RotateCcw, Play, Sparkles, Loader2
} from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {isOwner && (
            <GenerateTaskListDialog
              open={generateOpen}
              onOpenChange={setGenerateOpen}
              onCreated={(id) => { setGenerateOpen(false); onSelectList(id); }}
            />
          )}
...
function GenerateTaskListDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (v: boolean) => void; onCreated: (id: number) => void }) {
  const { toast } = useToast();
  const [department, setDepartment] = useState("bakery");
  const [date, setDate] = useState(new Date().toISOString().split("T")[0]);

  const generateMut = useMutation({
    mutationFn: async (data: { department: string; date: string }) => {
      const res = await apiRequest("POST", "/api/task-lists/generate", data);
      return res.json();
    },
    onSuccess: (result: any) => {
      ...
      toast({
        ...
        description: `${itemCount} tasks created. ${ctx.teamSize || 0} team members, ${ctx.recipesConsidered || 0} recipes considered.`,
      });
      onCreated(result.taskList.id);
    },
    onError: (err: Error) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        <Button variant="outline" data-testid="button-generate-jarvis">
          <Sparkles className="w-4 h-4 mr-2" /> Generate with Jarvis
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Generate Task List with Jarvis
          </DialogTitle>
          <DialogDescription>
            Jarvis will analyze production needs, inventory levels, scheduled staff skills, and recent activity to create an optimized task list. The generated list is fully editable.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            ...
            <Select value={department} onValueChange={setDepartment}>
              <SelectTrigger data-testid="select-generate-department">
          ...
          <div>
            ...
            <Input
              ...
              data-testid="input-generate-date"
            />
          </div>
          <Badge variant="secondary" className="text-xs">
            Beta
        ...
        <DialogFooter>
          <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button
            onClick={() => generateMut.mutate({ department, date })}
            disabled={!department || !date || generateMut.isPending}
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {recipeId && !item.completed && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setLocation(`/recipes/${recipeId}/begin?taskListItemId=${item.id}`);
                          }}
                          data-testid={`button-begin-recipe-${item.id}`}
                        >
                          <Play className="w-3 h-3 mr-1" /> Begin
                        </Button>
                      )}
                      <Button
                        variant="ghost"
